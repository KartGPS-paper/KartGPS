...
class Graph(object):
    ...
    def add_edge(self, eid, frm, to, elb):
        ...
        # print(frm,to,elb)
        if (frm, to) in self.set_of_elb[elb]:
            return self
    ...
    def get_forward_root_edges(self, frm):
        result = []
        v_frm = self.vertices[frm]
        for to, e in v_frm.edges.items():

            if (not self.is_undirected) or v_frm.vlb <= self.vertices[to].vlb:
                result.append(e)
        return result
...
class Projected(list):
    ...
    Each element of Projected is a projection subgraph in one
    original data graph.
    """
...
class DFScode(list):
    ...
    #         return None
    #
    #     def get_forward_pure_edges(g, rm_edge, min_vlb, history):
    #         result = []
    ...
    #         return result
    #
    #     def get_forward_rmpath_edges(g, rm_edge, min_vlb, history):
    #         result = []
    ...
    #         flag, newto = False, 0,
    #         end = 0 if g.is_undirected else -1
    #         for i in range(len(rmpath) - 1, end, -1):
    #             if flag:
    #                 break
    #             for p in projected:
    #                 history = History(p)
    #                 e = get_backward_edge(g,history.edges[rmpath[i]],history.edges[rmpath[0]],history)
    #                 if e is not None:
    ...
    #         for p in projected:
    #             history = History(p)
    #             edges = get_forward_pure_edges(g,history.edges[rmpath[0]],min_vlb,history)
    #             if len(edges) > 0:
    #                 flag = True
    #                 newfrm = maxtoc
    #                 for e in edges:
    ...
    #             for p in projected:
    #                 history = History(p)
    #                 edges = get_forward_rmpath_edges(g,history.edges[rmpath_i],min_vlb,history)
    #                 if len(edges) > 0:
    #                     flag = True
