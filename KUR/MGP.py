...
class DFScode(list):
    ...
    def to_graph(self, gid=graph.VACANT_GRAPH_ID, is_undirected=True):
        ...
        for dfsedge in self:
            ...
            if vlb1 != graph.VACANT_VERTEX_LABEL:
                g.add_vertex(frm, vlb1)
            if vlb2 != graph.VACANT_VERTEX_LABEL:
...
class PDFS(object):
    ...
    def __init__(self, gid=graph.VACANT_GRAPH_ID, edge=None, prev=None):
...
class MGP(object):
    ...
    def _read_graphs(self):
        self.graphs = graph.read_graphs(self._database_file_name)
...
if __name__ == '__main__':
    mgp = MGP("graph.data.simple.5")
    mgp._read_graphs()
    mgp._generate_1edge_frequent_subgraphs()